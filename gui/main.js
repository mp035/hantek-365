...
function startHantek(mode = "", relative = false) {
  ...
  mainWindow.webContents.send("statusMessage", {
    data: "Hantek process (re)started.\n",
  });

  hantek.on("exit", function (code, signal) {
    try {
      mainWindow.webContents.send("statusMessage", {
        data: `Hantek process exited with code ${code} and signal ${signal}\n`,
      });
    } catch (err) {
...
ipcMain.on("saveDataResponse", async (event, data) => {
  try {
    const result = await dialog.showSaveDialog(mainWindow, {
      filters: [
        { name: "Comma Separated Variable", extensions: ["csv", "txt"] },
      ],
    });
